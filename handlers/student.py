...
from keyboard import main_menu, slots_keyboard, cancel_keyboard, teacher_confirm_keyboard
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "slots":
        available = get_available_slots()
        if not available:
            await query.edit_message_text("❌ Hozircha ochiq slot yo'q.", reply_markup=main_menu())
        else:
        ...
        if result is None:
            await query.edit_message_text("⚠️ Bu so'rov allaqachon ko'rib chiqilgan yoki sizga tegishli emas.")
            return
        # Notify student
        try:
            await context.bot.send_message(
            ...
            pass
        await query.edit_message_text(f"✅ {result['student_name']} uchun bandlov tasdiqlandi.")
        ...
        try:
            await context.bot.send_message(
                chat_id=result["student_telegram_id"],
                text="❌ *Bandlovingiz rad etildi.*\n\nO'qituvchi so'rovingizni qabul qilmadi. Boshqa vaqtni tanlang: /slots",
                parse_mode="Markdown",
            )
        except Exception:
            pass
        await query.edit_message_text(f"❌ {result['student_name']} uchun bandlov rad etildi.")
